...
function updateChildren(el, oldChildren, newChildren) {
  ...
  while (oldStartIndex <= oldEndIndex && newStartIndex <= newEndIndex) {
    if (!oldStartVnode) {  // 乱序比对中旧数组节点可能为空
      ...
      newStartVnode = newChildren[++newStartIndex]// oldChildren->abc  newChildren->obnysck
    }
  }

  if (newStartIndex <= newEndIndex) {  // 插入新数组头尾指针间的节点，例如 push 以及 unshift 情况
    for (let i = newStartIndex; i <= newEndIndex; i++) {//oldchild--abc  newchild--abcde
  ...
  if (oldStartIndex <= oldEndIndex) {  // 删除旧数组头尾指针间的节点，例如 pop 以及 shift 情况
    for (let i = oldStartIndex; i <= oldEndIndex; i++) {//oldchild--abcde  newchild--abc
